 ...
 * @author ItsEternity
 */
public class TeamAcceptCommand extends ChildCommand {

    private final TeamsPlugin plugin;

    public TeamAcceptCommand(Command parent, TeamsPlugin plugin) {
        super(parent, "accept");
        this.plugin = plugin;
    }
    ...
    public void execute(Player player, String... args) {
        ...
        if (team.hasInvite(player.getUniqueId())) {
            ...
            Message.JOINED_TEAM.send(player, target.getName());

            for (UUID uuid : team.getMembers()) {
                Player member = Bukkit.getPlayer(uuid);
                if (member != null && !player.equals(member)) {
                    Message.JOINED_TEAM_BROADCAST.send(member, player.getName());
                }
            }
